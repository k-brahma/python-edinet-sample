...
def load_companies(file_path=None):
    ...
    try:
        ...
        logger.error(f"企業情報ファイルが見つかりません: {file_path}")
        return []
    except json.JSONDecodeError:
        logger.error(f"企業情報ファイルのJSON形式が不正です: {file_path}")
        return []
    except Exception as e:
        logger.error(f"企業情報ファイルの読み込みエラー: {e}")
        return []
...
def create_filtered_documents_with_pandas(all_documents_csv, companies, output_csv):
    ...
    try:
        ...
        logger.debug(f"secCodeカラムのデータ型: {df['secCode'].dtype}")
        ...
        try:
            ...
            logger.info(f"エラーが発生したため、元のファイルを {output_csv} にコピーしました")
            return True
        except:
            logger.error("ファイルのコピーにも失敗しました")
            return False
...
async def main_async():
    ...
    # 結果ディレクトリの作成
    os.makedirs(base_settings.RESULTS_DIR, exist_ok=True)
    ...
    if all_documents:
        ...
        for company in companies:
            ...
            if company_docs:
                ...
                doc_type_str = ", ".join([f"{t}: {c}件" for t, c in doc_types.items()])
                logger.info(
                    f"- {company_name}（{seccode}）: {len(company_docs)}件 ({doc_type_str})"
                )
